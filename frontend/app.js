// Render Dynamic Backend API endpoint configuration
const API_BASE_URL = window.location.origin.includes('localhost') 
  ? 'http://localhost:5000/api/items' 
  : `${window.location.origin}/api/items`;

const WHATSAPP_NUMBER = "94771234567"; // Replace with your friend's real WhatsApp number

// Mock Data for instantaneous local fallback if DB isn't loaded yet
const mockData = [
...
async function fetchProducts(category = 'all') {
  ...
  try {
    ...
    if (!response.ok) throw new Error("Fallback to mock data");
    const data = await response.json();
    renderGrid(data.length ? data : mockData.filter(i => category === 'all' || i.category === category));
  } catch (err) {
    // Elegant fallback to mock dataset for smooth frontend demo presentation
