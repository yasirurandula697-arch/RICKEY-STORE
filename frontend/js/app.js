...
// firebase.initializeApp(firebaseConfig); // මෙය ඔයාගේ HTML එකේ script tag එකේ දැනටමත් තියෙනවා නම් නැවත දාන්න එපා.
...
// 4. CHECK ADMIN STATUS ON LOAD
document.addEventListener("DOMContentLoaded", () => {
    const adminStatus = localStorage.getItem("isAdminLoggedIn");
    
    // ඇඩ්මින් කෙනෙක් ලොග් වෙලා නම් පමණක් පෙන්වන මෙනු/බටන්ස්
    if (adminStatus === "true") {
        console.log("Admin Panel Active");
        // මෙතනදී ඔයාට පුළුවන් ඇඩ්මින්ට විතරක් පේන බොත්තම් පෙන්වන්න
        // උදා: document.getElementById('admin-actions').style.display = 'block';
    }
});

// 5. ඇඩ්මින්ට අයිතමයක් අලුතින් එකතු කිරීමට සරල Function එකක්
async function addNewItem(itemData) {
    if (localStorage.getItem("isAdminLoggedIn") !== "true") {
        alert("Unauthorized!");
        return;
    }
    // මෙතන ඔයාගේ API එකට POST රික්වෙස්ට් එකක් යවන්න
